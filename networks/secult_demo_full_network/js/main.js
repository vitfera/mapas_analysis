...
function nodeActive(a) {
    ...
    if (f.attributes) {
  		...
  		if (config.informationPanel.imageAttribute) {
  			image_attribute="image"//config.informationPanel.imageAttribute;
  		}
        e = [];
        temp_array = [];
        g = 0;
        ...
        let text = f.attributes["data"];
        if (text != undefined) {
            const regex = /avatarSmall': \{'url': '([^']+)'\}/;
            const match = text.match(regex);
            if (match) {
                console.log(match[1])
                f.attributes[image_attribute] = match[1];

            } else {
                image_attribute = false;
            }
        }

        console.log(f.attributes[image_attribute])
